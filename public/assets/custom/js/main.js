...
$(document).ready(function(){
    ...
    $(".resetBtn").click(function(){
        ...
        $('#result').html("<h2>Click the textbox, and copy the content on the paragraph.</h2>");//reset header
...
function Timer(event){
    ...
    if(timer == 0 && event.which!=13 && stopped == 0){
        $('#timmer').html("<h2>Time elapsed: " + seconds/10 + " seconds.</h2>");
        timer = 1;
...
function startTime () {
    ...
    $('#timmer').html("<h2>Time elapsed: " + seconds/10 + " seconds.</h2>");
/*
    $('#time-count').html("<h2>Time elapsed: " + seconds/10 + " seconds.</h2>");
*/
}
...
function Errors(event){
    ...
    if(String.fromCharCode(event.which) == wordLib[currWord][currIndex]){
        ...
        if(currWord == 9 && currIndex == wordLib[currWord].length){ //@change 4 to #words - 1
            StopTime();
            return;
        }
...
function StopTime(){
    ...
    $('#result').html("<h3>Your typing speed in net WPM: " + netWPM + " words per minute." + "Errors: " + numErrors + " Time:" + seconds/10);
